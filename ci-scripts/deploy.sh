...
ssh -o "StrictHostKeyChecking no" makeen@$(get_azureenvironment).cloudapp.net 'source deploy.env; docker-compose -f docker-deploy.yml rm -f -v makeen; docker-compose -f docker-deploy.yml pull makeen; docker-compose -f docker-deploy.yml -p makeen up --build --remove-orphans -d'