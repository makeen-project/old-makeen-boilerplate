...
ssh -o "StrictHostKeyChecking no" makeen@$(get_azureenvironment).cloudapp.net 'source deploy.env; docker-compose rm -f ; docker-compose pull -f docker-deploy.yml; docker-compose -f docker-deploy.yml -p makeen up --build --remove-orphans -d'